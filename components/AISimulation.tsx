...
const AISimulation: React.FC = () => {
  ...
  useEffect(() => {
    if (!isEditing) {
      setEditValues({
        ...
        title:
          config.ai_simulation_title ||
          'Virtual <br />\n<span class="inline-block mt-6 bg-pullmanBrown text-white px-8 py-4 border border-white/10 shadow-xl">Simulation.</span>',
        descriptionQuote:
          config.ai_simulation_quote ||
          `"Experimenta el diseño antes de que exista."`,
        descriptionBody:
          config.ai_simulation_body ||
  ...
  const aiSimColors = editColors ||
    config.theme_colors?.ai_simulation || {
      bg: "bg-navyDark",
      text: "text-white",
    };
  ...
  const handleSave = async () => {
    await updateLocalConfig({
      ...
      theme_colors: {
        ...config.theme_colors,
        ai_simulation: { bg: aiSimColors.bg, text: aiSimColors.text },
      },
    });
    setIsEditing(false);
  };
  ...
  return (
    <section
      ...
      style={{
        color: aiSimColors.text?.startsWith("#") ? aiSimColors.text : undefined,
      }}
    >
          ...
          <div className="space-y-12 animate-in slide-in-from-left-8 duration-1000">
            {isEditing ? (
              <RichTextEditor
                ...
                onChange={(val) =>
                  setEditValues({ ...editValues, subtitle: val })
                }
            ...
            {isEditing ? (
              <RichTextEditor
                ...
                onChange={(val) =>
                  setEditValues({ ...editValues, descriptionQuote: val })
                }
              ...
              <p
                ...
                dangerouslySetInnerHTML={{
                  __html: editValues.descriptionQuote,
                }}
              />
            )}
            ...
            {isEditing ? (
              <RichTextEditor
                ...
                onChange={(val) =>
                  setEditValues({ ...editValues, descriptionBody: val })
                }
            ...
            <div className="absolute -bottom-6 -left-6 bg-pullmanBrown p-6 border border-white/10 hidden md:block">
              {isEditing ? (
                <RichTextEditor
                  ...
                  onChange={(val) =>
                    setEditValues({ ...editValues, badgeText: val })
                  }
